...
Gunakan SQL ini hanya jika test write ke `/vector-webhook` terlanjur menambah row ke PGVector. Jalankan lewat Postgres credential yang sama dengan workflow n8n.
...
WHERE v.collection_id = c.uuid
  ...
  AND v.text IN (
    'Frontend smoke test VectorDB setelah n8n publish. Abaikan dokumen ini jika terlihat di hasil retrieval.',
